...
/**
 * Process a single alert job from the alert queue
 */
export const processAlertJob = async (job, mailTransporter) => {
  ...
  // 2. Dispatch email and record delivery status
  const activeTransporter = mailTransporter !== undefined ? mailTransporter : transporter;
  ...
  if (activeTransporter) {
    try {
      ...
      // Log confirmed success in database
      await query(
        `INSERT INTO email_logs (id, monitor_id, recipient, subject, body, status, sent_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [crypto.randomUUID(), monitorId, recipientEmail, subject, bodyHtml, 'sent', new Date()]
      );
    } catch (mailErr) {
      const attemptsMade = job?.attemptsMade ?? 0;
      const maxAttempts = job?.opts?.attempts ?? config.alertRetryCount ?? 1;
      const isLastAttempt = (attemptsMade + 1) >= maxAttempts;

      if (isLastAttempt) {
        console.error(`[Alert Worker] SMTP dispatch permanently failed after ${attemptsMade + 1} attempt(s) to ${recipientEmail}:`, mailErr);

        // Terminal failure: record in database with error message
        await query(
          `INSERT INTO email_logs (id, monitor_id, recipient, subject, body, status, error, sent_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
          [crypto.randomUUID(), monitorId, recipientEmail, subject, bodyHtml, 'failed', mailErr.message || String(mailErr), new Date()]
        );
      } else {
        console.warn(`[Alert Worker] SMTP dispatch attempt ${attemptsMade + 1}/${maxAttempts} failed to ${recipientEmail} (${mailErr.message}). Scheduling retry.`);
      }

      // Re-throw so BullMQ records job failure and triggers retry backoff
      throw mailErr;
    }
  } else {
    console.log(`[Alert Worker] [MOCK EMAIL ALERT SENT TO ${recipientEmail}]\nSubject: ${subject}\nBody: ${bodyText}\n`);

    // SMTP not configured - log explicitly as 'mocked'
    await query(
      `INSERT INTO email_logs (id, monitor_id, recipient, subject, body, status, sent_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [crypto.randomUUID(), monitorId, recipientEmail, subject, bodyHtml, 'mocked', new Date()]
    );
  }
};
